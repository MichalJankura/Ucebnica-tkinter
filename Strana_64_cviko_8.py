...
def draw_sad_face(x,y):
    ...
    canvas.create_oval(x-20, y-20, x-10, y-10, fill='black')
    canvas.create_oval(x+20, y-20, x+10, y-10, fill='black')
    canvas.create_arc(x-30, y+20, x+30, y-10, start=0, extent=180, fill='black')
...
def timer1():
    ...
    if pocet_bodov >= 5:  # If the number of points is greater than or equal to 20
        ...
        draw_smiley_face(200, 200)  # Draw a happy smiley face
        return
    if pocet_bodov <= -5:  # If the number of points is greater than or equal to 20
        print("Máš isté rezervy!")  # Print a congratulatory message
        draw_sad_face(200, 200)  # Draw a happy smiley face
        return
